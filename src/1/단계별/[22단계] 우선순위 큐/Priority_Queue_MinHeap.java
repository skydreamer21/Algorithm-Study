...
public class Priority_Queue_MinHeap {
     ...
     */
    public static void Min_Heapify(int[] arr, int root, int size) {
        int left = 2*root;
        int right = 2*root+1;
        int min = root;
    ...
    public static void Min_Heapify_up(int[] arr, int child) {
        int root = child/2;
        if (root!=0 && arr[child]<arr[root]) {
            swap(arr,root,child);
            Min_Heapify_up(arr,root);
        }
    }

    public static void Build_MinHeap (int[] arr, int size) {
        // 마지막으로 자식노드를 가지는 노드
        int root = size/2;
        while (root>0) Min_Heapify(arr,root--,size);
    }
